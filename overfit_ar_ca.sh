...
WORKDIR_ROOT=$SCRATCH/frederieke/mars-ar
...
export DATA_DIR=$SCRATCH/frederieke/md_data
...
FRESH="${FRESH:-1}"
if [[ "${FRESH}" == "1" && "${SLURM_ARRAY_TASK_ID}" == "0" ]]; then
...
python - <<PYEOF
import pandas as pd
df = pd.read_csv('splits/4AA_train.csv', index_col='name')
...
srun python -m scripts.train \
  ...
  --data_dir $DATA_DIR/4AA_sims_processed \
  --model_dir "${WORKDIR_ROOT}" \
  --batch_size 1 --crop 128 --val_repeat 1 --epochs 20000 \
  --ckpt_freq 1000 --wandb \
  --run_name "${RUN_NAME}" --wandb_name "${WANDB_NAME}" \
  --msm_num_states 10 --clusters_per_batch 10 --samples_per_cluster 80 \
