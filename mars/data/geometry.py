...
from ..vendored.openfold.tensor_utils import batched_gather


def atom14_to_atom37(atom14: np.ndarray, aatype):
...
def atom37_to_torsions(all_atom_positions, aatype):
    if isinstance(all_atom_positions, np.ndarray):
    ...
    chi_atom_indices = torch.as_tensor(_CHI_ATOM_INDICES, device=aatype.device)
...
def atom14_to_frames(atom14):
    ...
    prot_frames = Rigid.from_3_points(c_coords, ca_coords, n_coords)
    B, T = atom14.shape[:2]
    rots = torch.eye(3, device=atom14.device).expand(B, T, 3, 3).clone()
    rots[:, :, 0, 0] = -1
    rots[:, :, 2, 2] = -1
...
def _build_chi_atom_indices():
    """Atom indices needed to compute chi angles for all residue types.
    Returns a list of shape [residue_types=21, chis=4, atoms=4].
    """
    chi_atom_indices = []
    ...
    chi_atom_indices.append([[0, 0, 0, 0]] * 4)  # Unknown residue
    return chi_atom_indices


_CHI_ATOM_INDICES = _build_chi_atom_indices()